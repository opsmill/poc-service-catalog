...
```shell
poetry run infrahubctl schema load schemas/ --wait 20
poetry run infrahubctl object load data/
```

### 6. Connect Read-Only Repository

Infrahub can connect to external repositories to fetch scripts, templates, or other resources. This command connects the demo repository as a read-only source.
...
Now that you have set up the demo, you can explore the service catalog features and functionalities. For a guided walkthrough, refer to the [walkthrough documentation](walkthrough).
